...
export default function AuditLogInternal() {
  ...
  const handleExport = async () => {
    ...
    await exportToPDF('Audit Log Report', ['Actor UID', 'Action', 'Entity Type', 'Entity ID', 'Timestamp', 'Metadata'], rows, `audit-log-${Date.now()}.pdf`);
    if (user?.uid) {
      void recordAuditLog({
        actorUid: user.uid,
        action: 'export_pdf',
