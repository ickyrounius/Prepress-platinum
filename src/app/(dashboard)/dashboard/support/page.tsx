...
export default function SupportDashboard() {
  ...
  useEffect(() => {
    const q = query(collection(db, 'proses_support_b'));
    const unsub = onSnapshot(q, (snapshot) => {
      const items: Array<Record<string, unknown>> = [];
      snapshot.forEach((doc) => {
        items.push({ id: doc.id, sourceType: 'SUPPORT', ...doc.data() });
      });
  ...
  const stats = [
    { title: "Total Task", value: 452, icon: Wrench, color: "amber" },
    { title: "Blueprint", value: 310, icon: Printer, color: "blue" },
    { title: "GMG Proof", value: 85, icon: Palette, color: "purple" },
    { title: "CNC Block", value: 57, icon: Cube, color: "emerald" },
    { title: "On Process", value: 45, icon: Clock, color: "sky" },
    { title: "Closed Task", value: 395, icon: CheckCircle, color: "emerald" },
    { title: "Hold/Pending", value: 12, icon: WarningCircle, color: "rose" },
  ];
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm transition-all hover:shadow-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Blueprint, GMG & CNC Progress</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <motion.div variants={itemVariants} className="bg-white border border-slate-100 rounded-[3rem] p-10 shadow-sm hover:shadow-md transition-shadow group">
          <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight">Tren Task (30 Hari)</h3>
        ...
        <motion.div variants={itemVariants} className="bg-white border border-slate-100 rounded-[3rem] p-10 shadow-sm hover:shadow-md transition-shadow group">
          <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight">Beban Kerja Support</h3>
      ...
      <motion.div variants={itemVariants} className="bg-white border border-slate-100 rounded-[3rem] p-8 shadow-sm">
        <div className="mb-6">
          <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight">Kanban Proses Support</h3>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Board khusus divisi Support Design</p>
        </div>
