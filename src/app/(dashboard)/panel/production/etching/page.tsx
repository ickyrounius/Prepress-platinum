...
export default function ETCHINGPanel() {
  const { role } = useAuth();
      ...
      <div className="flex flex-col gap-2 mb-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 bg-amber-500 text-white rounded-[1.25rem] flex items-center justify-center shadow-lg shadow-amber-200">
            <Flame weight="bold" size={24} />
          </div>
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Panel Operator ETCHING</h1>
            <p className="text-sm font-medium text-slate-400 uppercase tracking-widest text-[10px]">Metal Etching & Chemical Milling</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-8">
          <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 space-y-6">
            <div className="flex items-center gap-2 mb-2">
              <Info weight="bold" className="text-amber-500" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1">
                ...
                <input required onChange={(e) => updateFormField('no_b', e.target.value)} value={(formData.no_b as string) || ''} className="w-full p-4 border-2 border-slate-100 rounded-2xl bg-white text-sm font-bold text-slate-700 focus:border-amber-500 outline-none transition-all placeholder:text-slate-300 shadow-sm" placeholder="Contoh: B1, B2..." />
              </div>
              ...
              <div className="space-y-1">
               ...
               <div className="flex items-end mb-1">
                 <div className="px-4 py-2 bg-amber-50 rounded-xl border border-amber-100 flex items-center gap-2">
                    <Stack weight="fill" size={14} className="text-amber-500" />
                    <span className="text-[9px] font-black text-amber-600 uppercase tracking-tighter">Precision Etching Control</span>
                 </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2">
              <ClipboardText weight="bold" className="text-amber-500" />
              Catatan Operator
            </label>
            <textarea 
              onChange={(e) => updateFormField('catatan_operator', e.target.value)} 
              value={(formData.catatan_operator as string) || ''}
              className="w-full p-5 border-2 border-slate-100 rounded-[2rem] bg-white text-sm font-bold text-slate-700 focus:border-amber-500 outline-none transition-all h-32 resize-none placeholder:text-slate-300 shadow-sm" 
              placeholder="Kendala cairan, status plat logam, atau detail pengerjaan etching..."
            ></textarea>
          </div>
        </div>
      </GlobalInputForm>
