...
export default function CTPPanel() {
  const { role } = useAuth();
    ...
    <div className="p-4 sm:p-8 space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col gap-2 mb-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 bg-emerald-500 text-white rounded-[1.25rem] flex items-center justify-center shadow-lg shadow-emerald-200">
            <Printer weight="bold" size={24} />
          </div>
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Panel Operator CTP</h1>
            <p className="text-sm font-medium text-slate-400 uppercase tracking-widest text-[10px]">Computer To Plate Production</p>
          </div>
        </div>
      </div>

      <GlobalInputForm
        title="Update Progress CTP"
        collectionName="proses_ctp_b"
        autoGenPrefix="CTP-PROC"
      >
        <div className="space-y-8">
          <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1">
                ...
                <input required onChange={(e) => updateFormField('no_b', e.target.value)} value={(formData.no_b as string) || ''} className="w-full p-4 border-2 border-slate-100 rounded-2xl bg-white text-sm font-bold text-slate-700 focus:border-emerald-500 outline-none transition-all placeholder:text-slate-300 shadow-sm" placeholder="Contoh: B1, B2..." />
              </div>
              
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 text-emerald-500">Bekerja Sebagai</label>
                <div className="w-full p-4 border-2 border-emerald-100 rounded-2xl bg-emerald-50 cursor-default text-sm font-black text-emerald-600 shadow-sm flex items-center justify-between">
                  <span className="uppercase tracking-wider">{role || 'MEMUAT...'}</span>
                  <span className="text-[9px] text-emerald-400 font-bold bg-white px-2 py-0.5 rounded-full border border-emerald-100 tracking-widest uppercase">Auto</span>
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 text-emerald-600">Status Prepress</label>
                <select required 
                  ...
                  className="w-full p-4 border-2 border-emerald-100 rounded-2xl bg-white text-sm font-black text-emerald-600 focus:border-emerald-500 outline-none transition-all cursor-pointer shadow-sm"
                >
               ...
               <div className="flex items-end mb-1">
                 <div className="px-4 py-2 bg-emerald-50 rounded-xl border border-emerald-100 flex items-center gap-2">
                    <Stack weight="fill" size={14} className="text-emerald-500" />
                    <span className="text-[9px] font-black text-emerald-600 uppercase tracking-tighter">Terintegrasi dengan Realtime Database</span>
                 </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2">
              <ClipboardText weight="bold" className="text-emerald-500" />
              Catatan Operator
            </label>
            <textarea 
              onChange={(e) => updateFormField('catatan_operator', e.target.value)} 
              value={(formData.catatan_operator as string) || ''}
              className="w-full p-5 border-2 border-slate-100 rounded-[2rem] bg-white text-sm font-bold text-slate-700 focus:border-emerald-500 outline-none transition-all h-32 resize-none placeholder:text-slate-300 shadow-sm" 
