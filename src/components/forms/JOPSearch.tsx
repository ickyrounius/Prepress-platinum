...
export function JOPSearch({
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        ...
        const searchField = type === 'JOP' ? 'NO_JOP' : 'NO_JOS';
        ...
        // Note: For better search, we should use NO_JOP/NO_JOS as the index or do a 'where' query
        const q = query(
  ...
  const handleSelect = (item: SearchItem) => {
    ...
    const displayVal = (item[type === 'JOP' ? 'NO_JOP' : 'NO_JOS'] as string) || item.id;
  ...
  return (
    <div className={cn("relative space-y-1 w-full", className)} ref={dropdownRef}>
      {label && <label className="text-[10px] font-bold text-slate-400 uppercase">{label === 'NO_JOS' ? 'NO_JOS-D' : label}</label>}
      ...
      {isOpen && results.length > 0 && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="max-h-60 overflow-y-auto custom-scrollbar">
            {results.map((item) => {
              const displayId = (item[type === 'JOP' ? 'NO_JOP' : 'NO_JOS'] as string) || item.id;
              const subId = type === 'JOP' ? (item.NO_B as string) : (item.NO_JOD as string);
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between text-[10px] text-slate-500 mt-1">
                    ...
                    {subId && <span className="font-black text-indigo-500">{type === 'JOP' ? 'BLOK:' : 'ITEM:'} {subId}</span>}
                  </div>
                </button>
              );
            })}
      ...
      {isOpen && searchTerm.length >= 2 && results.length === 0 && !isSearching && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl p-4 text-center">
          <p className="text-sm text-slate-500">Tidak ditemukan nomor &quot;{searchTerm}&quot;</p>
        </div>
      )}
    </div>
  );
