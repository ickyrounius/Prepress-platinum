  ...
  try {
    ...
    if (role === 'spv') {
      ...
      updates = {
        ...
        HOLD_REASON: (workflowStatus === 'Hold' || workflowStatus === 'Reject') ? (payload.HOLD_REASON as string) : "",
        TINDAKAN_KOREKTIF: (payload.TINDAKAN_KOREKTIF as string) || "",
        REVISI_KE: parseInt(String(payload.REVISI_KE || 0)) || 0
      };
      ...
      let finalProgress = (payload.ST_PRO_NO_B || payload.ST_PRO_JOP || currentData.ST_PRO_NO_B || currentData.ST_PRO_JOP) as string;
      ...
      updates = {
        ...
        ST_APPROVAL: payload.ST_APPROVAL as string,
        ST_QC: payload.ST_QC as string,
        QC_USER: payload.QC_USER as string,
        HOLD_REASON: (finalWorkflow === 'Reject') ? (payload.HOLD_REASON as string) : "",
        TINDAKAN_KOREKTIF: (payload.TINDAKAN_KOREKTIF as string) || "",
        REVISI_KE: parseInt(String(payload.REVISI_KE || 0)) || 0
      };
    ...
    if (finalStatus === "CLOSED" || finalStatus === "CANCEL") {
      try {
        ...
        await recordAuditLog({
          actorUid: (payload.operator_id as string) || "SYSTEM",
